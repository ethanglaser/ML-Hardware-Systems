...
For both. (1) and (2) above, there are functionality tests provided in this assignment to check that your implementations are correct. Note that you are required to use python code to implement each of those functions--you can't just call a library function to perform Winograd for example. The only exception to that is singular value decomposition (SVD) where you are allowed to use `numpy` or other libraries to perform the decomposition itself. for `fft` in particular, please describe your implementation in your writeup.
