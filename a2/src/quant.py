...
class SymmetricQuantFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, k, specified_scale=None, specified_zero_point=None):
        ...
        output = torch.clamp(ste_round().forward(None, x / scale), -(2 ** (k - 1)), 2 ** (k - 1) - 1)
        return output
    ...
    def get_quantization_params(self, saturation_min, saturation_max):
        ...
        with torch.no_grad():
            if self.is_symmetric:
                ...
                zero_point = ste_round().forward(None, (-saturation_min) / (saturation_max - saturation_min) * (2 ** self.quant_bits - 1))
...
def quantize_weights_bias(w, qconfig, fake_quantize=False):
    ...
    w_q = qconfig.quantize_with_min_max(w, torch.min(w), torch.max(w), fake_quantize)
...
def conv2d_linear_quantized(
    ...
    if module.bias is not None:
        ...
        module.bias.data = b_qconfig.quantize_with_min_max(module.bias.data, torch.min(module.bias.data), torch.max(module.bias.data), fake_quantize=True)
    module.weight.data = w_qconfig.quantize_with_min_max(module.weight.data, torch.min(module.weight.data), torch.max(module.weight.data), fake_quantize=True)
    temp = quantize_activations(x, a_qconfig, fake_quantize=True)
    y = module(temp)
    return y
